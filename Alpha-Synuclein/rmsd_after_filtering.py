...
# Define the paths to the AlphaFold folder and the reference amyloid PDB directory
alphafold_dir = r"./PDBs/aligned_structures"  # Update this to your actual AlphaFold directory path
amyloid_dir = r"./PDBs/alpha_synuclein_unique_amyloids"  # Update this to your actual reference directory path

segment_length = 15
n_range = 80

# Use glob to get a list of all PDB files in the AlphaFold directory
alphafold_dirs = glob.glob(os.path.join(alphafold_dir, "*.pdb"))
# Use glob to get a list of all PDB files in the amyloid directory
amyloid_dirs = glob.glob(os.path.join(amyloid_dir, "*.pdb"))
...
df = pd.DataFrame(np.inf, index=[f"{n}-{n + segment_length - 1}" for n in range(1, n_range + 1)],
                  columns=[os.path.splitext(os.path.basename(amyloid_file))[0] for amyloid_file in amyloid_dirs])

# Loop through each AlphaFold PDB file
for alphafold_file in tqdm(alphafold_dirs):
...
df.to_csv('rmsd_values_final.csv')
